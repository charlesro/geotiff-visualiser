...
1. **Polygons** — load polygons from a database (local Python/DuckDB engine via `POST /query`, geometry as WKT) or from a GeoJSON / zipped shapefile, then select the ones to analyse (in the list or by clicking the map).
